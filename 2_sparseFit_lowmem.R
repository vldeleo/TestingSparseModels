...
if (is.null(yOOS) | is.null(XOOS)){
  write.csv(c("yval", "Xval"), file = paste("ModelResults/Rep_", l, "_FitInput.csv", sep = ""))
  write.csv(c("ytest", "Xtest"), file = paste("ModelResults/Rep_", l, "_TestInput.csv", sep = "")) 
}
...
###################################################################
### now run it ###
### this function will not load all simulated data into memory at the same time and crash the session ###
...
#*** Optional, if I want to use an already existing sample of extra data ***
